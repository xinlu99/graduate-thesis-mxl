...
第\ref{chap:ch3_unified_model}章已经建立天基网络可信协同计算卸载的统一场景、基础模型与可信状态刻画，第\ref{chap:ch4_system_resource_allocation}章进一步从系统层研究多节点长期可信协同，为任务层决策提供候选节点约束、负载与信誉摘要等系统级上下文。在此基础上，本章聚焦任务到达瞬间的节点侧实时卸载问题，讨论卫星终端如何在有限观测条件下完成本地执行或目标节点卸载选择。该问题的核心困难在于：链上公共摘要具有可信性与可追溯性，但受共识、传播和确认过程影响并非实时；本地观测具有更强时效性，却难以呈现全局负载结构；候选节点上的并发开销又会随任务组合和资源竞争状态动态变化，难以依靠固定解析模型准确刻画。

链上状态“可信但非实时”的属性意味着，任务层策略不能仅依赖链上公共摘要，还需要结合本地即时观测和并发风险预测形成面向当前任务的决策依据。为此，本章构建“聚类联邦学习预测 + DQN 在线决策”的任务层方法：前者在不上传原始执行日志的前提下学习候选节点的并发风险，后者将预测开销及其不确定性转化为本地执行或目标节点卸载动作。

本章按“问题提出—代价建模—预测学习—卸载决策—实验验证”的顺序展开。第~5.2 节给出任务层卸载框架、综合代价模型与并发开销建模；第~5.3 节介绍基于聚类联邦学习的并发开销预测方法；第~5.4 节给出 FL-DQN 卸载决策算法；第~5.5 节从预测精度、端到端性能、鲁棒性和消融四个方面验证所提方法的有效性。
...
问题的关键并不在于“是否存在可卸载节点”，而在于“面对动态环境时，节点如何做出正确卸载动作”。新任务到达卫星终端（Satellite Terminal， ST）后，终端需要在很短的时间窗内综合任务计算量、输入规模、时延阈值、本地资源余量、链路可达性和候选节点状态，判断任务应本地执行还是卸载至某个 SEN。与地面有线或稳定无线环境不同，星间链路带宽、传播时延和可见窗口随轨道运动持续变化，节点状态感知存在天然滞后。这意味着不少现有调度方法所默认的“调度器能拿到候选节点准确、即时、全局一致状态”的前提，在天基场景下并不成立——星间链路传播时延明显、链路窗口有限，节点间状态同步天然异步，硬件能力和任务组合的差异使周期性交换状态也难以形成足够细粒度的系统全貌。继续沿用依赖完美状态信息的集中式思路，实际部署时往往面临观测滞后、通信开销高以及模型泛化不足等问题。

并发开销是其中最容易被低估、却又最直接决定卸载效果的关键变量。当多个任务在同一候选 SEN 上并发执行时，新任务的进入不仅会延长自身完成时延，还会通过 CPU 时间片争用、缓存冲突、内存带宽竞争和 I/O 排队等机制干扰既有任务的推进，造成执行队列的系统级性能退化。换言之，并发开销并不是附着于新任务的单一额外代价，而是同时作用于新任务与存量任务的耦合扰动。如果卸载策略仍依据候选节点平均负载、瞬时 CPU 利用率或链上静态摘要做判断，就很容易把任务送往“表面空闲、实际拥挤”的目标节点，最终导致时延放大、能耗增加和拥塞累积。

因此，本章不再重复求解系统层协同资源分配问题，而是在第\ref{chap:ch4_system_resource_allocation}章形成的候选约束、负载均衡导向和信誉上下文基础上，进一步研究单任务到达时的节点侧动作选择。联盟链在本章中的作用相应体现为可信状态与模型凭据的维护：它通过链上公共摘要 $G(t)$ 提供候选节点负载、信誉和账本延迟等系统级信息，并通过第\ref{chap:ch3_unified_model}章定义的模型摘要 $M(t)$ 维护联邦预测模型的版本号、哈希和所属簇信息。二者共同构成任务层可利用的扩展摘要 $G^{\mathrm{task}}(t)$，但并不消除传播、确认与同步过程引入的信息陈旧性。因此，节点侧卸载决策需要在链上可信摘要、本地即时观测和并发风险预测之间形成互补。
...
针对上述约束，本章采用“并发开销预测 + 在线卸载决策”的分层思路。聚类式联邦学习在不上传原始执行日志的前提下训练并发开销预测模型，学习候选节点在不同任务组合和运行背景下的潜在拥塞风险；预测得到的并发开销及其不确定性，与任务特征、本地状态和链上扩展摘要共同输入深度 Q 网络（Deep Q-Network， DQN）代理，生成节点侧离散卸载动作。采用“预测—决策”分层结构的原因在于：并发开销同时受硬件架构、任务类型和执行上下文影响，固定解析公式难以稳定刻画；与此同时，天基任务卸载的候选动作集合通常有限，离散动作空间的 DQN 能够在保持较低在线推理开销的同时，将预测先验转化为面向单任务决策的策略增益。

本章的主要贡献体现在三个方面。首先，从任务层视角形式化刻画并发开销，将新任务进入成本与存量任务受扰成本同时纳入代价表达；其次，构建适用于 Non-IID 与隐私受限场景的分层聚类联邦学习框架，并借助链上模型摘要维护预测模型的版本一致性与可追溯性；最后，将并发开销预测值及其不确定性显式融入 DQN 状态空间与奖励设计，使节点侧策略具备面向潜在资源竞争的前瞻感知能力。后续实验从预测层、端到端性能、鲁棒性与消融四个维度验证所提方法的有效性。
...
本节首先给出任务级卸载场景的系统结构与综合代价模型，随后对并发开销进行形式化定义和可计算近似，并将节点侧观测整理为统一状态表征，最终建立任务层 MDP 表达，为后续联邦预测和强化决策提供建模基础。
...
图~\ref{fig:ch5_system_framework} 给出了任务层卸载方法的双时间尺度结构。图中数字 1--5 表示在线任务卸载链路，包括账本检查、模型版本确认、任务卸载、任务计算和任务结束上链；字母 a--e 表示后台联邦模型维护链路，包括初始模型分发、本地训练、模型聚合、模型更新和区块上链。两条链路对应不同时间尺度：前者面向单任务到达后的快速决策，后者面向历史日志驱动的异步模型更新。
...
第~5.2 节已经给出并发开销定义，本节回答的问题是：在隐私受限和样本异构的天基边缘场景下，并发开销如何被有效学习。下面分别说明监督样本构造、轻量预测器设计、聚类式联邦训练流程，以及模型版本的链上可信维护。
...
第~5.3 节解决了并发开销的预测问题，本节讨论的是如何把这种风险感知转化为实时卸载动作。预测模型的细节不再重述，焦点放在 DQN 如何读取预测先验、构造奖励约束并完成在线交互上。
...
为验证 FL-DQN 任务卸载方法的有效性，本节从预测层和决策层两方面展开实验。预测层主要考察并发开销预测精度、训练收敛过程、置信校准结果和异构环境下的误差变化；决策层主要比较平均任务时延、系统吞吐量、SLA 完成率和 P99 尾时延等指标，并分析链上摘要陈旧性、动作选择偏好、效率--公平性折中和模块消融结果。
