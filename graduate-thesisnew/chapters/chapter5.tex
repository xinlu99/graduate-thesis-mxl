...
在完成并发开销预测模型构建与 DQN 卸载策略设计之后，本节进一步说明二者在节点侧在线决策过程中的协同方式。与图~\ref{fig:ch5_system_framework} 所展示的系统级双时间尺度架构不同，图~\ref{fig:ch5_closed_loop} 聚焦于单个任务到达时的实时卸载决策闭环。其核心思想是：当新任务到达节点后，节点首先融合任务特征、本地资源状态、链上公共摘要 $G(t)$ 以及模型摘要 $M(t)$，确定当前可行动作集合；随后，依据候选目标节点所属簇调用最新的联邦并发开销预测模型，对每个候选动作产生并发开销估计及其不确定性；最后，将预测先验与原始状态特征共同输入 DQN，由 DQN 输出本地执行或星间卸载动作。
...
如图~\ref{fig:ch5_closed_loop} 所示，FL-DQN 的在线决策过程可以理解为“状态感知—预测增强—动作选择—反馈更新”的闭环。首先，任务到达后，节点提取任务规模、计算量、截止期、任务类型和优先级等特征，并同步读取本地 CPU、内存、队列长度、剩余能量和链路状态等高频状态信息。与此同时，节点从链上公共摘要 $G(t)$ 中获取候选边缘节点的低频可信状态，包括负载水平、信誉信息、链路统计和链上时延估计等，并根据模型摘要 $M(t)$ 确认当前可用的预测模型版本。

在获得候选动作集合后，节点并不直接将原始状态输入 DQN 进行动作选择，而是先针对每个候选动作 $a\in\mathcal{A}$ 构造动作相关特征向量 $\bm{x}(s_t,a)$。该特征向量同时包含新任务属性、候选目标节点状态、链路条件以及链上摘要信息。随后，节点根据候选目标所属簇调用对应的联邦并发开销预测模型 $f_{\theta_f}$，快速推理得到该动作可能引入的并发开销预测值 $\widehat{\Delta}_i(s_t,a)$ 及其不确定性 $\widehat{\sigma}_i^2(s_t,a)$。这些预测结果刻画了新任务被接纳或卸载后可能造成的额外队列竞争、调度扰动和存量任务影响。

完成预测后，节点将任务特征、本地资源状态、链上扩展摘要以及各候选动作对应的预测先验进行拼接，形成扩展状态表示 $s_t$。DQN 主网络 $Q(\cdot;\theta_q)$ 在该扩展状态上计算各候选动作的动作价值，并按照 $\varepsilon$-greedy 规则选择本地执行或目标节点卸载动作。动作执行后，环境返回任务完成时延、能耗、并发扰动和完成结果，系统据此计算即时奖励 $r_t$，并将交互样本 $(s_t,a_t,r_t,s_{t+1})$ 写入经验回放池。随后，DQN 从经验池中采样 mini-batch 更新主网络，并按照固定步长同步目标网络参数，从而在持续交互中逐步优化节点侧卸载策略。

上述流程的关键在于，并发开销预测模型并不直接替代卸载策略，而是作为前瞻性先验增强 DQN 的状态表征。传统 DQN 通常基于当前可观测状态进行动作价值估计，难以显式感知新任务接纳后对自身队列和存量任务造成的潜在扰动。本文通过将候选动作对应的并发开销预测值 $\widehat{\Delta}_i(s_t,a)$ 及不确定性 $\widehat{\sigma}_i^2(s_t,a)$ 纳入扩展状态，使 DQN 在估计动作价值时能够同时考虑即时资源状态和未来竞争风险。由此，在线卸载决策从单纯的反应式选择转化为融合预测先验的前瞻式动作评估。

为了更清楚地展示“预测先验如何进入在线动作选择”，算法~\ref{alg:ch5_fldqn_online} 对 FL-DQN 的节点侧执行流程给出伪代码描述。与图~\ref{fig:ch5_closed_loop} 相比，算法描述更强调实现层面的控制流：即先针对候选动作逐一构造动作相关特征并调用预测模型，再将预测结果拼接进扩展状态，最后依据 DQN 完成动作选择和参数更新。
...
预测层能提供风险先验只是前提，仍需进一步回答的问题是：这些先验能否转化为任务级卸载性能的实际提升。本文在不同平均并发任务数下，对 FL-DQN 与多种基线方法在平均任务时延、系统吞吐量、SLA 完成率和 P99 尾时延四项指标上做了对比。其中，平均任务时延与系统吞吐量结果见图~\ref{fig:ch5_service_quality_ab}，SLA 完成率与 P99 尾时延结果见图~\ref{fig:ch5_service_quality_cd}。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{images/4-new-7-ab.pdf}
    \caption{不同并发强度下的卸载决策性能}
    \label{fig:ch5_service_quality_ab}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{images/4-new-7-cd.pdf}
    \caption{不同并发强度下的卸载决策稳定性}
    \label{fig:ch5_service_quality_cd}
\end{figure}
...
图~\ref{fig:ch5_service_quality_ab}（a） 显示，平均并发任务数增加时所有方法的平均时延都在上升——并发越高，候选节点内部的 CPU 时间片争用、缓存冲突、内存带宽竞争和 I/O 排队就越严重，端到端完成时间因此持续增加。但不同方法的时延增长斜率差距明显：Heuristic 和 DQN-only 在高并发区间上升较快，仅靠当前负载或即时观测状态难以准确判断候选节点的未来拥塞；FL-DQN 的曲线则平缓得多，因为预测先验让策略在卸载前就能识别潜在拥塞节点，把任务被送往“表面空闲但实际竞争严重”目标的概率压低了。
...
吞吐量方面，图~\ref{fig:ch5_service_quality_ab}（b） 显示，并发增加导致各方法的吞吐量都不同程度下降，原因是节点队列积压和任务执行时间延长压低了单位时间任务完成数。Centralized-DQN 整体最高，FL-DQN 紧随其后，且明显优于其余三类——这说明“链上扩展摘要 + 本地观测 + 联邦预测先验”的组合在不依赖全局实时透明信息的前提下已经接近集中式策略的处理能力。

SLA 完成率方面，图~\ref{fig:ch5_service_quality_cd}（c） 显示，低并发区间各方法差距很小，资源较充足时大多数任务的时延约束本就容易满足；并发持续增大后，Heuristic 和 DQN-only 的完成率下降最快，反映其在高负载下更容易出现拥塞误判和任务超时。FL-DQN 的下降斜率最缓，预测先验提前规避高风险动作的效果在资源竞争加剧时尤为明显。

P99 尾时延方面，图~\ref{fig:ch5_service_quality_cd}（d） 关注的是最差那部分任务的完成情况，对时延敏感型天基业务尤其关键。所有方法的尾时延都随并发任务数上升，但 FL-DQN 的增幅相对较小，在高并发区间显著低于 DQN-only 与 Heuristic——其改善的不仅是平均服务质量，也包括对极端拥塞样本的稳定性抑制。

把图~\ref{fig:ch5_service_quality_ab}、图~\ref{fig:ch5_service_quality_cd} 与表~\ref{tab:ch5_e2e_performance_summary} 放在一起看，FL-DQN 的优势在中高并发区间更突出。这与方法设计目标一致：轻载下候选节点之间的资源竞争尚不显著，预测模块带来的边际收益有限；负载逼近拥塞边界时，并发开销预测才能为 DQN 提供有效的前瞻信息，让策略在时延、吞吐、SLA 与尾时延上都表现出更好的稳定性。
\subsection{消融分析}
