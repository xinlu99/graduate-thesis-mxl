...
\BITSetup{
  ...
  info = {
    ...
    UDC = 540,
    title = 基于区块链的卫星终端试验任务数据管理与计算卸载技术研究,
    % 如需覆盖竖排标题，请配置以下选项。
    % 下面的例子展示了如何在竖排标题中使用垂直或者旋转的英文。
    % verticalTitle = {形状记忆聚氨酯{L } {T } {X }的合成 \rotatebox[origin=c]{-90}{Feng Kaiyu} 及其在织物中的应用},
    titleEn = {Data Management and Computing Offloading Technology Scheme for Satellite Terminal Test Task Based on Blockchain},
    author = 毛心璐,
    authorEn = Xinlu Mao,
...
% 定义居中自适应宽度列类型 C（用于 tabularx 环境）
\newcolumntype{C}{>{\centering\arraybackslash}X}
% 第三章表格（tabularx）：P{width} 为定宽居中列，Y 为左对齐可伸缩列；与全文已定义的 C（居中 X）区分
